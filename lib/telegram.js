...
function processUpdate(updateJson) {
  ...
  if (messageHandler) messageHandler.catch((err) => console.log("Telegram bot: Error: " + err.message));
}
  ...
  if (messageProcessed) {
    return messageProcessed.then((response) => {
      if (!response) {
        ...
        return sendApiRequest("sendMessage", Object.assign({
          chat_id: message.chat.id,
          reply_to_message_id: message.message_id
        }, response));
      }
...
function sendApiRequest(method, data) { return new Promise((resolve, reject) => {
