...
function sendApiRequest(options) {
  return new Promise((resolve, reject) => {
    if (options.post) {
      twitterApiClient.post(options.path, options.params, callback);
    } else {
      twitterApiClient.get(options.path, options.params, callback);
    }
...
function doStuff() { co(function* () {
  ...
  tweets = tweets.filter((tweet) => {
    if (tweet.in_reply_to_user_id_str || /labrat/i.test(tweet.source)) {
      return false;
    } else {
      return true;
    }
...
function processTweet(tweet) { return co(function* () {
  ...
  if (tweetContainsMedia.entities.media) {
    for (var i = 0; i < tweetContainsMedia.entities.media.length; i++) {
      if (process.env["LABRAT_ENV"] === "dev") {
        console.log(`DEBUG: Send photo ${tweetContainsMedia.entities.media[i].media_url_https} .`);
      } else {
        yield telegram.sendApiRequest("sendPhoto", {
          chat_id: config.twitter.channelName,
          photo: tweetContainsMedia.entities.media[i].media_url_https,
...
function convertEntities(tweet) { return co(function* () {
  ...
  if (entities.length === 0) {
    result = telegram.tgHtmlEscape(result);
  }
